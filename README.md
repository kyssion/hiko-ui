...
也不用这么抽象, 直接指定是怎么生成就好了, 其实本质上一个组件的渲染其实氛围自定义渲染和自动化渲染和默认渲染 .
默认渲染 : 按照json的格式生成dom节点信息 => 一个组件其实有很多参数 , 参数其实就是绑定了渲染的逻辑. 
自定义渲染 : 自定义渲染的逻辑, 生成相关的ui => 组件中的某一个属性或关联数据源的一个数据key. 
自动化: 拥有预设的渲染能力可以进行自动化的渲染 . 
...
动态状态器 :
出了有上面的这些逻辑 ,  并且还要附加创建组件的逻辑 .



事件发出的是消息 -> 消息中有样式属性和数据属性的变化 . 
...
```json
{
  ...
  /** 独有的属性*/
  "节点类型": "静态 vs 动态",
  "动态类型生成函数": "一个函数 , 入参的是当前的数据 , 出参的是是否要生成 ,  动态节点输入必须是一个数组或者可以生成数组的值的函数",
  "ID": "唯一标识 , 用来标记当前节点 , 动态生成的节点的子ID是动态生成的. ",
  "getData": "获取当前数据的get方法",
  "setData": "设置当前的函数数据的方法",
  "render": "基于当前的数性生成页面"
}
```
...
https://www.csjplatform.com/union_pangle/open/api/mediation/app_conf?user_id=459&role_id=459&app_id=5001121
